...
| GPRD-008 | MUST  | When a component exposes loading state for an interactive root, loading MUST suppress activation while preserving appropriate focus and busy semantics; consumer activation handlers MUST NOT be called for suppressed loading activation events.                         | Library policy | Current       |
| GPRD-009 | MUST  | Component hooks that compose consumer event handlers with library event handlers MUST call the consumer handler first unless documented platform semantics require suppression.                                                                                           | Library policy | Current       |
| GPRD-010 | MUST  | Optional library synthetic behaviour MUST NOT run when the consumer handler prevents default; library-owned state bookkeeping MAY still run after consumer handlers when required to preserve the documented component contract.                                          | Library policy | Current       |
