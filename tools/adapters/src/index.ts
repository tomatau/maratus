...
export async function renderInlineCssVarsAdapter(input: InlineCssVarsAdapterInput) {
  const { componentName, inlineStyleLiteral, inlineStyleVarName, sourceComponent } = input
  const templatePath = join(import.meta.dir, "..", "templates", "inline-css-vars.tsx.tmpl")
  const template = await readFile(templatePath, "utf8")
  const sourceFunctionExport = `export function ${componentName}(`
  const sourceComponentInline = sourceComponent.replace(sourceFunctionExport, `function ${componentName}(`)
