...
func newComponentsDirModel(defaultValue string, suggestions []string, existingDirs []string) *componentsDirModel {
	...
	for _, suggestion := range suggestions {
		...
		options = append(options, componentsOption{
			Value:       suggestion,
...
func (m *componentsDirModel) Init() tea.Cmd {
	m.cursorBlink = true
...
func (m *componentsDirModel) View() string {
	if m.done {
...
func (m *componentsDirModel) filteredOptions() []componentsOption {
	query := strings.ToLower(strings.TrimSpace(m.query))
	if query == "" {
		return m.options
	}

	filtered := make([]componentsOption, 0, len(m.options))
...
func (m *componentsDirModel) clampCursor() {
	filtered := m.filteredOptions()
	if len(filtered) == 0 {
		m.cursor = 0
		return
...
func (m *componentsDirModel) autocomplete() {
	query := strings.TrimSpace(m.query)
	if query == "" {
		filtered := m.filteredOptions()
		if len(filtered) > 0 {
...
func (m *componentsDirModel) result() string {
