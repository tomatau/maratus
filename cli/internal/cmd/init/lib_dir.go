...
func AskLibDir(cmd *cobra.Command, configRoot string, srcDir string) (string, error) {
	...
	for _, suggestion := range suggestions {
		existingDirs = append(existingDirs, suggestion)
	}
...
func newLibDirModel(defaultValue string, suggestions []string, existingDirs []string) *libDirModel {
	...
	for _, suggestion := range suggestions {
		...
		options = append(options, libOption{
			Value:       suggestion,
...
func (m *libDirModel) Init() tea.Cmd {
	m.cursorBlink = true
...
func (m *libDirModel) View() string {
	if m.done {
...
func (m *libDirModel) filteredOptions() []libOption {
	query := strings.ToLower(strings.TrimSpace(m.query))
	if query == "" {
		return m.options
	}

	filtered := make([]libOption, 0, len(m.options))
...
func (m *libDirModel) clampCursor() {
	filtered := m.filteredOptions()
	if len(filtered) == 0 {
		m.cursor = 0
		return
...
func (m *libDirModel) autocomplete() {
	query := strings.TrimSpace(m.query)
	if query == "" {
		return
	}

	matches := make([]string, 0, len(m.existingDirs))
	for _, dir := range m.existingDirs {
		if strings.HasPrefix(strings.ToLower(dir), strings.ToLower(query)) {
...
func (m *libDirModel) result() string {
	value := strings.TrimSpace(m.chosenValue)
	if value == "" {
		value = strings.TrimSpace(m.query)
	}
	if value == "" {
