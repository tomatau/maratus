...
| REQ-007 | MUST   | When the field has visible errors, `Error` must expose a stable `id` for the rendered error content.                                                                | WAI-ARIA 1.2 `aria-errormessage`; WCAG 2.2 SC 3.3.1; WCAG 2.2 SC 4.1.2              | Current       |
...
| REQ-011 | SHOULD | When an error message appears after user interaction, `Error` should expose `role="alert"` so assistive technologies can announce the message without moving focus. | WAI-ARIA 1.2 `alert`; WCAG 2.2 SC 3.3.1; WCAG 2.2 SC 4.1.3                          | Current       |
| REQ-012 | MUST   | Field relationship props must preserve native form control attributes such as `name`, `required`, `type`, `pattern`, and constraint validation state.               | HTML Standard form control infrastructure; HTML Standard constraint validation      | Current       |
| REQ-013 | MUST   | Field relationship wiring must not require `FieldRoot` to receive, store, or update the control value.                                                              | HTML Standard form control infrastructure                                           | Current       |
| REQ-014 | SHOULD | When `FieldRoot` receives `activeErrors`, those error keys should be the field's current active errors.                                                             | WAI-ARIA 1.2 `aria-invalid`; WAI-ARIA 1.2 `aria-errormessage`; WCAG 2.2 SC 3.3.1    | Current       |
| REQ-015 | SHOULD | When `FieldRoot` does not receive `activeErrors`, the field control should derive active error keys from the browser constraint validation API.                     | HTML Standard constraint validation; WAI-ARIA 1.2 `aria-invalid`; WCAG 2.2 SC 3.3.1 | Current       |
| REQ-016 | SHOULD | When detailed supporting content is supported, the field control should reference that content with `aria-details`.                                                 | WAI-ARIA 1.2 `aria-details`; WCAG 2.2 SC 1.3.1                                      | Potential     |

## Product Requirements

| ID      | Requirement                                                                                                      | Applicability |
| ------- | ---------------------------------------------------------------------------------------------------------------- | ------------- |
| PRD-001 | Export `FieldRoot`, `Label`, `Description`, and `Error` from the field package entry point.                      | Current       |
...
| PRD-004 | `Label`, `Description`, and `Error` must render the corresponding content from the closest ancestor `FieldRoot`. | Current       |
...
- `Error` renders visible error messages by looking up visible error keys in `errorMap`.
