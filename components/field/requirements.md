...
- Role-aware non-native text controls for custom widgets
...
| REQ-012 | MUST   | `TextControl` render props must allow consumers to render native form controls that preserve `name`, `type`, `required`, `disabled`, `readOnly`, `min`, `max`, `minLength`, `maxLength`, `pattern`, `autocomplete`, and browser constraint validation behaviour. | HTML Standard form control infrastructure; HTML Standard constraint validation               | Current       |
...
| REQ-016 | MUST   | When `FieldRoot` receives `isRequired`, `TextControl` render props must expose `required` for native controls so browser required semantics and validation remain available.                                                                                     | HTML Standard form control infrastructure; HTML Standard constraint validation               | Current       |
| REQ-017 | MUST   | When `FieldRoot` receives `isReadOnly`, `TextControl` render props must expose `readOnly` for the native controls listed in [Native readonly controls](#native-readonly-controls).                                                                               | HTML Standard form control infrastructure; WCAG 2.2 SC 4.1.2                                 | Current       |
...
| REQ-020 | MUST   | When role-aware text controls are supported, `TextControl` must limit supported widget roles to `textbox`, `searchbox`, and `combobox`.                                                                                                                          | WAI-ARIA 1.2 widget roles; WCAG 2.2 SC 4.1.2                                                 | Current       |
| REQ-021 | MUST   | When `TextControl` renders a role-aware non-native text control, the render props must expose the requested supported `role` and preserve field relationship attributes.                                                                                         | WAI-ARIA 1.2 widget roles; WAI-ARIA 1.2 `aria-describedby`; WAI-ARIA 1.2 `aria-errormessage` | Current       |
| REQ-022 | MUST   | When a role-aware text control is required, `TextControl` render props must expose `aria-required="true"` for supported text roles.                                                                                                                              | WAI-ARIA 1.2 `aria-required`; WCAG 2.2 SC 3.3.2; WCAG 2.2 SC 4.1.2                           | Current       |
| REQ-023 | MUST   | When a role-aware text control is readonly, `TextControl` render props must expose `aria-readonly="true"` for supported text roles.                                                                                                                              | WAI-ARIA 1.2 `aria-readonly`; WCAG 2.2 SC 4.1.2                                              | Current       |
| REQ-024 | MUST   | When `TextControl` renders a non-native `textbox` or `searchbox`, it must require or pass through the text entry attributes listed for that role in [Role-specific text control contracts](#role-specific-text-control-contracts).                               | WAI-ARIA 1.2 `textbox`; WAI-ARIA 1.2 `searchbox`; WCAG 2.2 SC 4.1.2                          | Current       |
| REQ-025 | MUST   | When `TextControl` renders a non-native `combobox`, it must require or pass through the popup relationship attributes listed for that role in [Role-specific text control contracts](#role-specific-text-control-contracts).                                     | WAI-ARIA 1.2 `combobox`; WCAG 2.2 SC 4.1.2                                                   | Current       |
| REQ-026 | MUST   | Future numeric controls that support non-native `spinbutton` must require or pass through the value attributes needed for a complete widget contract.                                                                                                            | WAI-ARIA 1.2 `spinbutton`; WCAG 2.2 SC 4.1.2                                                 | Potential     |
| REQ-027 | MUST   | Future choice controls that support non-native `listbox` must require or pass through the option relationship attributes needed for a complete widget contract.                                                                                                  | WAI-ARIA 1.2 `listbox`; WCAG 2.2 SC 4.1.2                                                    | Potential     |
| REQ-028 | MUST   | Future boolean controls that support non-native `checkbox` must require or pass through the checked state attributes needed for a complete widget contract.                                                                                                      | WAI-ARIA 1.2 `checkbox`; WCAG 2.2 SC 4.1.2                                                   | Potential     |
| REQ-029 | SHOULD | When a role-aware text control is rendered with a native element that exposes `ValidityState`, `TextControl` render props should keep native validity handlers available.                                                                                        | HTML Standard constraint validation; WCAG 2.2 SC 3.3.1                                       | Current       |
| REQ-030 | SHOULD | When a role-aware text control is rendered with a custom element that does not expose `ValidityState`, `TextControl` render props should expose a validity event wrapper that lets consumers pass custom validity state through the same validity handlers.      | HTML Standard constraint validation; WCAG 2.2 SC 3.3.1; WCAG 2.2 SC 4.1.2                    | Current       |
...
### Role-specific text control contracts
...
### Future non-native control roles

These roles remain expected future value-control surfaces. They are not part of `TextControl` because they represent numeric, choice, or boolean value contracts rather than text entry.

| Role         | Expected control surface                 | Required-capable | Readonly-capable | Notes                                                     |
| ------------ | ---------------------------------------- | ---------------- | ---------------- | --------------------------------------------------------- |
| `spinbutton` | Future numeric value control             | Yes              | Yes              | Numeric value widget with role-specific value attributes. |
| `listbox`    | Future single or multiple choice control | Yes              | Yes              | Selectable option collection.                             |
| `checkbox`   | Future boolean or choice-set control     | Yes              | Yes              | Checkable widget with role-specific checked state.        |

### Future role-specific control contracts
...
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| PRD-001 | Export `FieldRoot`, `FieldProvider`, `FieldControl`, `TextControl`, `Label`, `Description`, and `ErrorMessage` from the field package entry point.                                                                                      | Current       |
...
| PRD-015 | `FieldRoot` must accept `isRequired` and `isReadOnly` as field-level state inputs for label state hooks and native text control props.                                                                                                  | Current       |
| PRD-016 | `TextControl` should support role-aware non-native text widgets through an explicit supported role API.                                                                                                                                 | Current       |
...
type TextControlRole = 'combobox' | 'searchbox' | 'textbox'

type TextControlRenderProps = FieldControlRenderProps & {
  name?: string
...
- Field hooks return named prop bags, such as `fieldRootProps`, `labelProps`, `fieldControlProps`, `textControlProps`, `descriptionProps`, and `errorMessageProps`.
- `isRequired` and `isReadOnly` are field-level state inputs. Native text control render props translate them to `required` and `readOnly`; label props translate them to `data-required` and `data-readonly`.
- `FieldControl` owns field-level relationship wiring for custom widget roots without selecting a widget role.
- Role-aware text controls are current scope. They must map `isRequired` and `isReadOnly` through `aria-required` and `aria-readonly`.
- Supported text control roles are intentionally narrow: `textbox`, `searchbox`, and `combobox`.
- `TextControl` render props expose `withValidity` next to `textControlProps` so custom controls can wrap events with a `ValidityState`-compatible object and still call the same validity handlers as native controls.
